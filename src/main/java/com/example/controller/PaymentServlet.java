...
import com.example.model.InvoicesMod;
import com.example.model.ProdMod;
import com.example.model.SendMail;
import com.example.model.ShipmentMod;
...
public class PaymentServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//check the given card number, cvv, expirydate, cardtype is correct or not
		    ...
		    if(rs.next()) {  //account exist
		    	...
		    	if(totalFinalAmount>rs.getInt("balanceAmount")) { //not enough blanace in a/c
		    		...
		    		pst3.setInt(2, customerAccountNo);  /// bank table lo unna account nnumber
		    		pst3.setInt(3, 1001); //ecommers account Number check in bank accounts
		...
		List<ProdMod> ListProducts = cart.getAllCartProductsByCustomerId(Integer.parseInt(session.getAttribute("customerId").toString()));
		...
		int totalBillAmount = 0;
			...
			for (ProdMod productOb : ListProducts) {
				...
				ShipmentMod shipment=new ShipmentMod();
	    ...
	    ////////////////////////
				...
				response.sendRedirect("/EcommersProject/Customer/Cart.jsp");
